...
async function buildResponse(requestedTime: string | undefined) {
  ...
  const { eventType, exactSlot, nearestSlot, availableTimes } = await getCalendlyAvailabilityDecision(normalizedRequestedTime);
  ...
  const message = exactAvailable
    ...
    : suggestedSlot
      ...
      : "Arthur does not have an open slot in the next 7 days for this event type.";
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("[API /calendly/check-slot GET] Error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("[API /calendly/check-slot POST] Error:", error);
    const message = error instanceof Error ? error.message : "Failed to check Google Calendar slot";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
