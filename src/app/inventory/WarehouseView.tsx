...
/**
 * Shared warehouse stock view — rendered by BOTH the Inventory "Overview" page
 * and the "Warehouse" page so they are literally identical and can never drift
 * (operator request). Product / Vendor / On hand / Unit cost / Value columns +
 * per-row Adjust and History, plus Log Refill (which used to live only on the
 * Overview page — kept here so that workflow isn't lost).
 */
...
type WarehouseRow = { product: Product; onHand: number };
type Movement = {
...
type MachineOption = { id: string; name: string; nayaxDeviceId: string | null; status: string };
type ProductOption = { id: string; name: string; sku: string; caseSize?: number };
...
export default function WarehouseView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [pRes, wRes, invRes] = await Promise.all([
      fetch("/api/inventory/products", { cache: "no-store" }),
      fetch("/api/inventory/warehouse", { cache: "no-store" }),
      fetch("/api/inventory?includeEmpty=0&page=1&pageSize=1", { cache: "no-store" }),
    ]);
    ...
    setRows(out);
    setMachines(invData.machines || []);
    setProductList(invData.productList || []);
    setLoading(false);
  }, []);
